...
def genXMatrix(f, y, BVs, h):
    
    size = len(y)
    xV = afRange(BVs[0][0]+h, BVs[-1][0], h)
    
    XList =  [f(BVs[0][1], y[0], y[1], xV[0])]
    XList += [f(y[i-1], y[i], y[i+1], xV[i]) for i in range(1, size-1)]
    XList += [f(y[-2], y[-1], BVs[-1][1], xV[-1])]
    
    return numpy.array(XList)
    
def genJMatrix(f, devs, x, BVs, h):
...
def solveBVP(fM, fV, h, BVs, dSize, guess):
    steps = int((BVs[-1][0] - BVs[0][0])/h)-1
    
    P = numpy.array([guess]*steps)
    
    for n in range(1000):
        a = genJMatrix(fM, dSize, P, BVs, h)
        b = genXMatrix(fV, P, BVs, h)
...
#     print fVal(-0.03255478, 0.06302292,  0.78967751, 1.6)   
    ...
    yList, norm, n = solveBVP(fMat, fVal, h, BVs, 3, 2.0)
     
    print yList, norm, n
    yList = yList.tolist()
    
    yList = [BVs[0][1]] + yList + [BVs[-1][1]] 
